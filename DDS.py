...
def runDDS():
    ...
    selector = GeneticSelectionCV(
        dds,  # algorithm name
        cv=5,  # cross validation generator -> determines cross validation splitting strategy => 5-fold cross validation.
        verbose=1,
        scoring="accuracy",
        max_features=5,
...
def graph():
    ...
    bars = (
        ...
        "MLP Accuracy",
        "DDS with Genetic Algorithm Accuracy",
    )
...
def predictType():
    ...
    for i in range(len(test)):
        ...
        if predict[i] == 2:
            text.insert(
                END, str(test[i]) + " : Decision Strategy is : Steering Angle\n"
            )
...
title = Label(
    main,
    text="A Driving Decision Strategy(DDS) Based on Machine learning for an autonomous vehicle",
)
title.config(bg="white", fg="teal")
...
trainButton = Button(
    main, text="Generate Train & Test Model", command=generateTrainTestData
)
